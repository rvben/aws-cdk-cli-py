...
def setup_test_environment():
    ...
    with patch("aws_cdk_bin.post_install.download_node", return_value=True):
        ...
        binary_dir = (
            Path(aws_cdk_bin.__file__).parent / "node_binaries" / system / machine
        )
...
def test_node_detection():
    ...
    if platform.system().lower() != "windows":
        ...
        result = subprocess.run(
            [aws_cdk_bin.NODE_BIN_PATH, "--version"], capture_output=True, text=True
        )
...
def test_cdk_detection():
    ...
    assert Path(aws_cdk_bin.CDK_SCRIPT_PATH).exists(), (

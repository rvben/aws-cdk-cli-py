...
def compare(version1, version2):
    ...
    if v1 is None or v2 is None:
        raise ValueError(f"Invalid version format: {version1 if v1 is None else version2}")
    ...
    if req.startswith('^'):
        ...
        if req_parsed[0] > 0:
            if version_parsed[0] != req_parsed[0]:
                return False
            return compare(version, req_version) >= 0 and version_parsed[0] == req_parsed[0]
        
        # Minor must match if major is 0 but minor > 0
        elif req_parsed[1] > 0:
            if version_parsed[0] != 0 or version_parsed[1] != req_parsed[1]:
                return False
            return compare(version, req_version) >= 0 and version_parsed[0] == 0 and version_parsed[1] == req_parsed[1]
